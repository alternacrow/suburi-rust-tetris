...
// 次世代を生成
fn gen_next_generation(inds: &[Individual]) -> Vec<GenoSeq> {
    let mut rng = rand::thread_rng();
    let mut genos = vec![];
    genos.extend(crossover(&inds)); // 交叉
    genos.extend(mutation(&inds)); // 突然変異
    genos.extend(selection(&inds)); // 選択
...
// 指定した範囲のデータを交換する
fn mem_swap_range<T>(a: &mut [T], b: &mut [T], range: std::ops::RangeInclusive<usize>) {
    for i in range {
